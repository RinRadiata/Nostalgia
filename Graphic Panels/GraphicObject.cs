...
public class GraphicObject
{
    ...
    private IEnumerator Fading(float target, float speed, Texture blend)
    {
        ...
        //the material must be apply to the image before any transition
        //must be remove after the transition is done
        if (renderer.material.name == DEFAULT_UI_MATERIAL)
        {
            Texture tex = renderer.material.GetTexture(MATERIAL_FIELD_MAINTEX);
            renderer.material = GetTransitionMaterial();
            renderer.material.SetTexture(MATERIAL_FIELD_MAINTEX, tex); //set the main texture to the material
        }
        ...
        renderer.material.SetFloat(MATERIAL_FIELD_BLEND, isBlending ? fadingIn ? 0 : 1 : 1);//straight blend using alpha and no effect on it
        ...
        while(renderer.material.GetFloat(opacityParam) != target)
        {
            ...
            if(isVideo)
                audio.volume = opacity; //adjust audio volume fading effect if it's a video
        ...
        if (target == 0)
            Destroy(); //destroy the graphic object if faded out
        else
        {
            DestroyBackgroundGraphicsOnLayer();
            renderer.texture = renderer.material.GetTexture(MATERIAL_FIELD_MAINTEX); //set the texture to the main texture of the material
            renderer.material = null; //take back to the default UI material
        }
    }
    ...
    public void Destroy()
    {
        if (layer.currentGraphic != null && layer.currentGraphic.renderer == renderer)
            layer.currentGraphic = null; //clear the current graphic reference in the layer

        if (layer.oldGraphics.Contains(this))
            layer.oldGraphics.Remove(this); //remove this graphic from the old graphics list so it won't be destroyed again and popup an error
